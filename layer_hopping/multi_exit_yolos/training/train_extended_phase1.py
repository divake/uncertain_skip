"""
Extended Phase 1 Training Script for Multi-Exit YOLOS

Trains detection heads for 50 epochs with frozen backbone.
...
def train_one_epoch(model, criterion, dataloader, optimizer, device, epoch, config):
    """Train for one epoch"""
    model.train()
    ...
    for batch_idx, (images, targets) in enumerate(pbar):
        ...
        # Forward pass (NO mixed precision - FP32 for stability)
...
def main():
    """Main extended Phase 1 training function"""

    # Load config
    config_path = 'configs/extended_phase1_config.yaml'
    with open(config_path, 'r') as f:
        config = yaml.safe_load(f)

    print(f"{'='*80}")
    print("EXTENDED PHASE 1 TRAINING: Multi-Exit YOLOS")
    print(f"{'='*80}")
    ...
    # Build optimizer (simple AdamW, no layer-wise LRs)
    print(f"\n{'='*80}")
